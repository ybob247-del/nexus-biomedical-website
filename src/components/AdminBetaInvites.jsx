...
/**
 * AdminBetaInvites — protected by AdminProtectedRoute (parent).
 * This component assumes the user is already authenticated.
 * It does NOT contain its own login form — auth is handled by AdminProtectedRoute.
 */
export default function AdminBetaInvites() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      // Include the session token from sessionStorage if present (fallback to cookie)
      ...
      if (response.ok && data.success) {
        ...
        setRecentInvites(prev => [
          { email, days, timestamp: new Date().toISOString() },
          ...prev.slice(0, 9),
        ]);
      } else {
  ...
  return (
    <div style={{
      ...
      <div style={{ maxWidth: '800px', margin: '0 auto' }}>
        ...
        <p style={{ color: '#B8D4E8', marginBottom: '3rem' }}>
          Grant beta access to users for all 6 platforms
        </p>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
              {recentInvites.map((invite, index) => (
                ...
                >
                  ...
                  <div style={{ fontSize: '0.9rem', color: '#B8D4E8', marginTop: '0.25rem' }}>
                    {invite.days} days &bull; {new Date(invite.timestamp).toLocaleString()}
                  </div>
                </div>
              ))}
            </div>
